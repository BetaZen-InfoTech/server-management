...
/**
 * Account → Sessions page. Shows the signed-in user's recent login
 * history so they can spot "was that me?" and flag anything suspicious.
 * Data populates on every successful login (password OR OTP) via
 * AuthService.RecordLoginSession — including IP, geolocation, and
 * parsed user-agent (browser/OS/device kind).
 */
interface Session {
  id?: string;
  email: string;
...
function DeviceIcon({ kind }: { kind?: string }) {
  ...
  switch (kind) {
    ...
    case "desktop":
      return <Monitor size={sz} className="text-blue-400" />;
    default:
