...
export default function ProfilePage() {
  ...
  useEffect(() => {
    apiClient
      ...
      .catch(() => {
        // Fall back to store values already hydrated above.
      })
  ...
  const saveProfile = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const d = res.data?.data || {};
      // Update the zustand store so the top-bar and sidebar re-render with
      // the new name/email immediately — no reload needed.
    ...
    <div className="max-w-3xl space-y-6">
      <div>
        ...
        <p className="text-panel-muted text-sm mt-1">
          Manage your login identity and password. Changes here apply to your account only.
        </p>
      </div>
      ...
      <Card>
        <form onSubmit={saveProfile} className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-panel-muted mt-1">
                Login identifier — fixed. Ask an admin to change it.
              </p>
            </div>
          </div>
          ...
          <div>
            ...
            />
            <p className="text-xs text-panel-muted mt-1">
              Used for sign-in and password recovery. Must be unique across the panel.
            </p>
          </div>
      ...
      <Card>
        <form onSubmit={changePassword} className="p-6 space-y-4">
          ...
          <div className="flex items-start gap-2 p-3 rounded-lg bg-amber-500/5 border border-amber-500/20 text-xs text-panel-muted">
            ...
            <div>
              Changing your password signs out every other active session. Your current browser stays
              signed in. Linux (SSH / FTP) credentials are rotated automatically when a Linux account
              exists for your username.
            </div>
          </div>
