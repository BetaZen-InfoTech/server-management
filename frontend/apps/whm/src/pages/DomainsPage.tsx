...
interface Domain {
  ...
  // 3.1.88 — records how the row was created. "manual" / "bulk_upload"
...
// pre-3.1.88 rows display cleanly without a migration.
function sourceMeta(source?: string): { label: string; cls: string; title: string } {
  switch (source) {
    case "bulk_upload":
      return {
        label: "Bulk",
        cls: "bg-purple-500/10 text-purple-300 border-purple-500/30",
        title: "Created from a CSV / XLSX bulk-upload row",
      };
    case "api":
      return {
        label: "API",
        cls: "bg-amber-500/10 text-amber-300 border-amber-500/30",
        title: "Created via the external programmatic API (POST /api/v1/external/domains)",
      };
    case "transfer":
      return {
        label: "Transfer",
        cls: "bg-cyan-500/10 text-cyan-300 border-cyan-500/30",
        title: "Restored from a server-to-server transfer",
      };
    case "manual":
    case "":
    case undefined:
      return {
        ...
        title: "Created via the Add Domain modal",
      };
    default:
      // Unknown future source (e.g. terraform, panel-sync) renders the
      // raw string so an integrator who adds a new origin doesn't have
      // to ship a UI change.
      return {
        label: source,
        cls: "bg-panel-bg/60 text-panel-muted border-panel-border",
        title: `Created via "${source}"`,
      };
  }
}
...
export default function DomainsPage() {
  ...
  // 3.1.88 — Source filter. Empty string = "All sources" (no filter);
  // "manual" / "bulk_upload" / "api" / "transfer" filter to that origin.
  ...
  const filtered = domains.filter((d) => {
    if (search) {
      ...
      if (!d.domain.toLowerCase().includes(s) && !(d.user || "").toLowerCase().includes(s)) {
        return false;
      }
    }
    if (sourceFilter) {
  ...
  // Per-source counts shown next to each option in the dropdown so the
  // operator sees what's available before clicking.
  const sourceCounts = useMemo(() => {
    const counts: Record<string, number> = { manual: 0, bulk_upload: 0, api: 0, transfer: 0 };
    for (const d of domains) {
      const k = d.source || "manual";
  ...
  const columns = [
    ...
    },
    {
      ...
      accessor: (d: Domain) => {
        ...
        return (
          <span
            className={`px-2 py-0.5 rounded text-[10px] font-medium border ${m.cls}`}
            title={m.title}
          >
            {m.label}
          </span>
          ...
          <div className="flex flex-col lg:flex-row lg:items-center gap-x-6 gap-y-2">
            <div className="flex items-center gap-1.5 flex-wrap">
              ...
              {[
                ...
                { key: "bulk_upload", label: "Bulk",      cnt: sourceCounts.bulk_upload || 0 },
                { key: "api",         label: "API",       cnt: sourceCounts.api || 0 },
                { key: "transfer",    label: "Transfer",  cnt: sourceCounts.transfer || 0 },
              ].map((opt) => {
                const active = sourceFilter === opt.key;
                const m = opt.key ? sourceMeta(opt.key) : null;
                return (
                  <button
                    key={opt.key || "all"}
                    type="button"
