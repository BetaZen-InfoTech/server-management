...
// /domains returns full domain records; we only read `domain` for the
// optional Domain dropdown on the Create form. The backend already scopes
// these to the logged-in vendor's tenant, so we don't need an owner/user
// field here (unlike the WHM variant which lets admins pick any vendor).
...
// MySQL roles mirror the WHM page exactly so the backend's role-validation
// logic doesn't diverge between panels.
const MYSQL_ROLES = [
...
export default function DatabasesPage() {
  ...
  // No vendor picker — the logged-in user's tenant is applied server-side.
  // Backend still derives the `<vendor>_` prefix from the caller; the user
  ...
  // Own domains — populates the optional Domain dropdown on the Create
  // form. Tagging a db with a domain helps the dashboard's per-site
  // grouping; it doesn't affect how the db can be used.
  ...
  // Used by the "Generate" button next to the password field — matches the
  // cPanel password helper so operators aren't tempted to type weak ones.
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Create database modal — no vendor picker; the caller's tenant is
          applied server-side. Optional domain dropdown uses /domains (own
          domains only). */}
          ...
          {availableDomains.length > 0 && (
            <div>
              <label className={labelClass}>
                Domain <span className="text-panel-muted text-xs font-normal">(optional)</span>
              </label>
              <select
                value={form.domain}
      ...
      {/* Remote Database Access modal — mirrors cPanel "Remote Database
          Access". Add IPs / CIDRs / hostnames / % wildcards; each one
          creates a MySQL host-scoped user and (for concrete IPs) opens the
          firewall. */}
      <Modal
