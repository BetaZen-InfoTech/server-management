            ...
            <div>
              ...
              <p className="text-xs text-panel-muted mt-1">
                {addForm.dns_provider === "cloudflare"
                  ...
                  : "Stays on the panel's own PowerDNS (default)."}
              </p>
            </div>
            {addForm.dns_provider === "cloudflare" && (
              <div>
                <label className="block text-sm font-medium text-panel-text mb-1.5">Cloudflare Proxy</label>
                <select
                  value={addForm.cf_proxy}
                  onChange={(e) => setAddForm({ ...addForm, cf_proxy: e.target.value })}
                  className={inputCls}
                >
                ...
                <p className="text-xs text-panel-muted mt-1">
                  {addForm.cf_proxy === "on"
                    ? "Web traffic through Cloudflare's proxy. Mail stays DNS-only."
                    : "Resolves straight to the origin — no proxy."}
                </p>
              </div>
            )}
          </div>
