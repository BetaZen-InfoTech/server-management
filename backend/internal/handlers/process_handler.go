...
func (h *ProcessHandler) List(c *fiber.Ctx) error {
	...
	procs, err := h.service.List(c.UserContext(), sort, limit); if err != nil { return response.InternalError(c, err.Error()) }
	return response.Success(c, procs)
}
func (h *ProcessHandler) Get(c *fiber.Ctx) error {
	pid := c.Params("pid"); proc, err := h.service.GetByPID(c.UserContext(), pid)
...
func (h *ProcessHandler) Kill(c *fiber.Ctx) error {
	...
	if err := h.service.Kill(c.UserContext(), pid, body.Signal); err != nil { return response.InternalError(c, err.Error()) }
	return response.SuccessMessage(c, "Signal sent", nil)
}
func (h *ProcessHandler) ListServices(c *fiber.Ctx) error {
	svcs, err := h.service.ListServices(c.UserContext()); if err != nil { return response.InternalError(c, err.Error()) }
...
func (h *ProcessHandler) ControlService(c *fiber.Ctx) error {
	...
	if err := h.service.ControlService(c.UserContext(), name, action); err != nil { return response.InternalError(c, err.Error()) }
