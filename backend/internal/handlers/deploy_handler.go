...
func (h *DeployHandler) List(c *fiber.Ctx) error {
	...
	deploys, total, err := h.service.List(c.UserContext(), page, limit); if err != nil { return response.InternalError(c, err.Error()) }
	return response.Paginated(c, deploys, page, limit, total)
}
func (h *DeployHandler) Get(c *fiber.Ctx) error {
	id := c.Params("id"); deploy, err := h.service.GetByID(c.UserContext(), id)
...
func (h *DeployHandler) Create(c *fiber.Ctx) error {
	...
	deploy, err := h.service.Create(c.UserContext(), &req); if err != nil { return response.InternalError(c, err.Error()) }
	return response.Created(c, deploy)
}
func (h *DeployHandler) Redeploy(c *fiber.Ctx) error {
	id := c.Params("id"); deploy, err := h.service.Redeploy(c.UserContext(), id)
...
func (h *DeployHandler) Rollback(c *fiber.Ctx) error {
	...
	_ = c.BodyParser(&body)
	_, err := h.service.Rollback(c.UserContext(), id, body.TargetCommit); if err != nil { return response.InternalError(c, err.Error()) }
	return response.SuccessMessage(c, "Rollback initiated", nil)
}
func (h *DeployHandler) Cancel(c *fiber.Ctx) error {
	id := c.Params("id"); if err := h.service.Cancel(c.UserContext(), id); err != nil { return response.InternalError(c, err.Error()) }
...
func (h *DeployHandler) Logs(c *fiber.Ctx) error {
	...
	logs, err := h.service.GetLogs(c.UserContext(), id, releaseID)
	if err != nil { return response.InternalError(c, err.Error()) }
...
func (h *DeployHandler) History(c *fiber.Ctx) error {
	...
	history, total, err := h.service.History(c.UserContext(), id, page, limit)
	if err != nil { return response.InternalError(c, err.Error()) }
...
func (h *DeployHandler) GitHubWebhook(c *fiber.Ctx) error {
	...
	if err := h.service.HandleGitHubWebhook(c.UserContext(), deployID, payload); err != nil { return response.BadRequest(c, err.Error(), nil) }
	return response.SuccessMessage(c, "Webhook processed", nil)
}
func (h *DeployHandler) Pause(c *fiber.Ctx) error {
	id := c.Params("id"); if err := h.service.Pause(c.UserContext(), id); err != nil { return response.InternalError(c, err.Error()) }
	return response.SuccessMessage(c, "Auto-deploy paused", nil)
}
func (h *DeployHandler) Resume(c *fiber.Ctx) error {
	id := c.Params("id"); if err := h.service.Resume(c.UserContext(), id); err != nil { return response.InternalError(c, err.Error()) }
